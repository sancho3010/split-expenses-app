...
from app.services.expense_service import CreateExpenseInput, create_expense
...
def test_crear_gasto_sin_especificar_split_divide_entre_todos():
    ...
    # 1 expense + 3 splits = 4 llamadas a db.add
...
def test_crear_gasto_especificando_algunos_miembros_divide_solo_entre_ellos():
    ...
    data = CreateExpenseInput(
        ...
        split_among_ids=[group.members[0].id, group.members[1].id],
    )
...
def test_crear_gasto_pagador_no_es_miembro_lanza_error():
    ...
    data = CreateExpenseInput(
        group_id=group.id,
        paid_by_id=uuid4(),  # ID que no pertenece al grupo
        description="Hotel",
        amount=Decimal("50.00"),
    )
...
def test_crear_gasto_miembro_split_no_pertenece_al_grupo_lanza_error():
    ...
    data = CreateExpenseInput(
        ...
        description="Hotel",
        amount=Decimal("50.00"),
        split_among_ids=[uuid4()],  # ID que no pertenece al grupo
    )
